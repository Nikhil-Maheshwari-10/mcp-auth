...
GOOGLE_REDIRECT_URI). Handles exactly one request — the browser redirect from
Google — extracts `code` and `state` from the query string, then shuts down.
...
class _CallbackHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:  # noqa: N802
        ...
        # Store results on the server instance so wait_for_callback() can read them.
        self.server.auth_code = params.get("code", [None])[0]
        self.server.state = params.get("state", [None])[0]
        self.server.error = params.get("error", [None])[0]

        # Send a minimal response so the browser doesn't show an error page.
    ...
    def log_message(self, format: str, *args) -> None:  # noqa: A002
        # Suppress the default per-request log line — auth.py prints its own status.
        pass
...
def wait_for_callback(port: int = 8080, timeout: int = 120) -> tuple[str, str]:
    ...
    Args:
        port:    Must match the port in GOOGLE_REDIRECT_URI.
    ...
    Raises:
        RuntimeError: If Google returned an error or the timeout expired.
    """
    server = HTTPServer(("localhost", port), _CallbackHandler)
    server.auth_code = None
    server.state = None
    server.error = None
    server.timeout = timeout  # used by handle_request() internally

    server.handle_request()  # blocks until exactly one request is received
    server.server_close()

    if server.error:
        raise RuntimeError(f"Google returned an OAuth error: {server.error}")
